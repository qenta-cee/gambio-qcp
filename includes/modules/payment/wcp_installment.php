...
class wcp_installment extends wcp_core {
	...
	function selection() {
		...
		$birthday = '<input type="date" name="wcp_birthday" required="required" value="'.$t_wcp_birthday.'" max="'.$maxDate.'"/>';
		...
		$payolutionTerms = '<input type="checkbox" name="wcp_payolutionsterms" required="required" value="'.$t_wcp_payolutionterms.'"/><span>'.MODULE_PAYMENT_WCP_INSTALLMENT_CONSENT1;
		if (strlen($mId)) {
			$payolutionTerms .= '<a id="wcp-payolutionlink" href="https://payment.payolution.com/payolution-payment/infoport/dataprivacyconsent?mId='.$mId.'" target="_blank">' . MODULE_PAYMENT_WCP_INSTALLMENT_LINK .'</a>';
		}else {
