...
class qcp_invoice extends qcp_core {
    ...
    function selection() {
	    ...
	    if(trim($_SESSION['qcp_birthday_invoice']) != '')
	    {
		    $t_qcp_birthday = $_SESSION['qcp_birthday_invoice'];
	    }

	    $maxDate = (date('Y')-18)."-".date('m')."-".date('d');
	    $birthday = '<input type="date" name="qcp_birthday_invoice" value="'.$t_qcp_birthday.'" max="'.$maxDate.'" class="form-control" />';
	    $birthDayField = array('title' => MODULE_PAYMENT_QCP_INVOICE_BIRTH, 'field' => $birthday);
	    $fields = array();
	    ...
	    $payolutionTerms = '<input type="checkbox" name="qcp_payolutionterms" value="'.$t_qcp_payolutionterms.'"/>&nbsp;<span>'.MODULE_PAYMENT_QCP_INVOICE_CONSENT1;
	    if (strlen($mId)) {
		    $payolutionTerms .= '<a id="qcp-payolutionlink" href="https://payment.payolution.com/payolution-payment/infoport/dataprivacyconsent?mId='.$mId.'" target="_blank"><b>' . MODULE_PAYMENT_QCP_INVOICE_LINK .'</b></a>';
	    }else {
		    $payolutionTerms .= MODULE_PAYMENT_QCP_INVOICE_LINK;
	    }
	    $payolutionTerms .= MODULE_PAYMENT_QCP_INVOICE_CONSENT2 . '</span>';

	    if ($terms && $provider == 'payolution') {
	    	array_push($fields, array('title' => MODULE_PAYMENT_QCP_INVOICE_TERMS_TITLE, 'field' => $payolutionTerms));
	    }
    ...
    function process_button() {
	    ...
	    $process_button_string = xtc_draw_hidden_field('qcp_birthday_invoice', $_POST['qcp_birthday_invoice']);
        ...
        if(qcp_core::constant("MODULE_PAYMENT_QCP_INVOICE_PROVIDER") == "RatePay")
        {
	...
	function pre_confirmation_check() {
		$maxDate = (date('Y')-18)."-".date('m')."-".date('d');
		if($_POST['qcp_birthday_invoice'] > $maxDate) {
			$error = MODULE_PAYMENT_QCP_INVOICE_BIRTHDAY_ERROR;
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&recheckok=' . false;
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		}
		if($_POST['qcp_birthday_invoice'] == '1000-01-01') {
			$error = MODULE_PAYMENT_QCP_INVOICE_EMPTY_BIRTHDAY_ERROR;
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&recheckok=' . false;
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		}
		if(!isset($_POST['qcp_payolutionterms']) && MODULE_PAYMENT_QCP_INVOICE_PROVIDER == 'payolution' && MODULE_PAYMENT_QCP_INVOICE_TERMS ) {
			$error = MODULE_PAYMENT_QCP_INVOICE_PAYOLUTION_ERROR;
			$payment_error_return = 'payment_error=' . $this->code . '&error=' . urlencode($error) . '&recheckok=' . false;
			xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, $payment_error_return, 'SSL', true, false));
		}
	}
